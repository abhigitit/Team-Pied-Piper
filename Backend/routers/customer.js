...
router.post("/editcustomer", auth, (req, res) => {
  ...
  let sql =
    "update customer set cust_name = ? , cust_email = ?, cust_phone = ?  where cust_id = ? ";
...
router.post("/reservation", auth, (req, res) => {
  ...
  let sql =
    "INSERT INTO reservation(room_id,cust_id,booking_date,start_date,end_date,amount) VALUES( ?, ? ,?,?,?,?)";
